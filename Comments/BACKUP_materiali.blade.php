<?php
#header("Location: /?404");
header("Content-Type:text/html; charset=utf-8");
?>
        <!DOCTYPE html>
<html>
<head>
    <title>Экспертная диагностика системы финансового управления</title>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
    <meta name="description" content="ITeam"/>
    <meta name="keywords" content="ITeam"/>

    <meta property="og:image" content="/images/logo.gif"/>
    <meta property="og:title" content="Разработка стратегии шаг за шагом"/>
    <meta property="og:description"
          content='Рекомендую бесплатный мастер-класс "Разработка стратегии шаг за шагом" консалтинговой компании iTeam'/>

    <link rel="shortcut icon" href="/favicon.ico"/>
    <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
    <style type="text/css">
        /** {*/
        /*background-attachment: scroll;*/
        /*background-color: rgba(0, 0, 0, 0);*/
        /*background-position: 95% center;*/
        /*background-repeat: no-repeat;*/
        /*margin: 0;*/
        /*padding: 0;*/
        /*}*/

        body {
            background-color: #EFEFEF;
            color: #003366;
            font: 20px Times New Roman, serif;
            margin: 0;
            max-width: 1280px;
            min-height: 100%;
            min-width: 960px;
            padding: 0;
        }

        /*#body {*/
        /*width: 800px;*/
        /*overflow: hidden;*/
        /*}*/

        #header {
            position: relative;
            height: 96px;
            margin: 0;
            width: 100%;
        }

        /*#logo {*/
        /*height: 70px;*/
        /*margin: 0 auto;*/
        /*width: 140px;*/
        /*}*/

        .logo {
            width: 200px;
            height: 70px;
            background: url('data:image/png;base64,') no-repeat scroll 0 -70px transparent;
            font: 70px/70px Arial, Verdana sans-serif;
            text-indent: 201px;
            display: block;
            overflow: hidden;
            border: 0 none;
            text-decoration: none;
            color: #202;
            letter-spacing: 35px;
        }

        .logo:hover {
            background: url('data:image/png;base64,') no-repeat scroll 0 0 transparent;
        }

        #logo:hover {
            cursor: default;
        }

        a.logo:hover {
            cursor: pointer;
        }

        #wrapper {
            clear: both;
            float: none;
        }

        s {
            text-decoration: none;
        }

        #page {
            background-color: #FFFFFF;
            margin: 40px 300px;
            padding: 40px;
            width: 1200px;
        }

        p {
            padding: 0 0 16px;
        }

        p big {
            color: rgb(204, 0, 0);
            font-size: 22px;
            line-height: 26px;
            margin: 0 0 32px;
            display: block;
        }

        #main .button_block {
            display: block;
            padding: 20px 0;
            position: relative;
        }

        #main .button_block a {
            background-color: rgb(221, 0, 17);
            border-radius: 8px;
            color: rgb(255, 255, 255);
            display: block;
            font: 18px/1 sans-serif;
            margin: 0 auto;
            padding: 8px;
            text-align: center;
            text-decoration: none;
            width: 250px;
        }

        #main .button_block a:hover {
            background-color: rgb(221, 0, 0);
        }

        #main {
            display: none;
        }

        a.btn-lg {
            background-color: #6A0101;
            color: #fff;
            font-weight: bold;
            border: none;
            width: 200px;
            font-size: 30px;
            margin: -10px 45px;
        }
    </style>
    <script src="https://code.jquery.com/jquery-1.10.2.js"></script>
</head>
<body>
<div id="page">
    <div id="body">
        <div id="header">

            <div id="logo">
                <a class="logo" href="/">ITeam</a>
            </div>

        </div>
        <div id="comercia">
            <h3>Спасибо за Ваше мнение!</h3>
            <h3>Немного терпения, презентация загрузиться через ... <span id="timer"></span></h3>
            <hr>
            <h1 style="color: darkred;text-align: center">СПЕЦИАЛЬНОЕ ПРЕДЛОЖЕНИЕ до 27 сентября!</h1>
            <div class="row">
                <div class="col-md-4">
                    <img src="/filemanager/userfiles/mkplanirovanieNEWW.png" width="80%;" style="margin-left: 115px">
                </div>
                <div class="col-md-8">
                    <h2 style="text-align: right; margin-top: 50px">Как организовать планирование в компании от
                        стратегического до
                        оперативного уровня</h2>
                    <p style="text-align: right">Мастер-класс для руководителей компаний и собственников бизнеса</p>

                    <div style="text-align: center; margin-top: 50px">
                        <div style="margin: 0px 145px"><span style="font-size: 26px"><del>4000</del></span> <span
                                    style="font-size: 40px; color: darkred">2400 &#8381;</span></div>
                        <br>
                        <a style="text-align: center" class="btn btn-default btn-lg"
                           href="https://iteam.ru/learn/webinar/effective_planning" target="_blank">Купить</a>
                    </div>
                </div>

            </div>
            <hr>
            <h3 style="text-align: center">Материалы будут доступны тут</h3>
        </div>
        <div id="main">

            <h3>Спасибо за ожидание</h3>
            <h3>Материалы доступны для скачивния ниже</h3>
            <hr>
            <h1 style="color: darkred;text-align: center">СПЕЦИАЛЬНОЕ ПРЕДЛОЖЕНИЕ до 27 сентября!</h1>
            <div class="row">
                <div class="col-md-4">
                    <img src="/filemanager/userfiles/mkplanirovanieNEWW.png" width="80%;" style="margin-left: 115px">
                </div>
                <div class="col-md-8">
                    <h2 style="text-align: right; margin-top: 50px">Как организовать планирование в компании от
                        стратегического до
                        оперативного уровня</h2>
                    <p style="text-align: right">Мастер-класс для руководителей компаний и собственников бизнеса</p>

                    <div style="text-align: center; margin-top: 50px">
                        <div style="margin: 0px 145px"><span style="font-size: 26px"><del>4000</del></span> <span
                                    style="font-size: 40px; color: darkred">2400 &#8381;</span></div>
                        <br>
                        <a style="text-align: center" class="btn btn-default btn-lg"
                           href="https://iteam.ru/learn/webinar/effective_planning" target="_blank">Купить</a>
                    </div>
                </div>

            </div>
            <hr>
            <a>Материалы для скачивания</a><br>
            <a>Материалы для скачивания</a><br>
            <a>Материалы для скачивания</a><br>
            <a>Материалы для скачивания</a><br>
        </div>
    </div>
</div>
</body>
</html>
<script>

    //событие загрузки страницы
    $(window).load(function (e) {
        setTimeout(function () {
            $('#comercia').css('display', 'none');
            $('#main').css('display', 'block')

        }, 30000); // in milliseconds
    });
    //--

    //таймер назад
    var count = 30;
    var counter = setInterval(timer, 1000); //1000 will  run it every 1 second
    function timer() {
        count = count - 1;
        if (count <= 0) {
            clearInterval(counter);
            //counter ended, do something here
            return;
        }

        //Do code for showing the number of seconds here
    }

    function timer() {
        count = count - 1;
        if (count <= 0) {
            clearInterval(counter);
            return;
        }

        document.getElementById("timer").innerHTML = count + " секунд"; // watch for spelling
    }

    //--
</script>